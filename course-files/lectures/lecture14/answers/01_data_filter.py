...
#     print('Name:', person[0], 'Occupation:', person[5])
#     # print('-' * 100)


'''
1. Create a function called get_list_of_zip_codes that takes a list of people as an argument and 
returns a list of zip codes (i.e., a list of strings)

2. Create a function called filter_by_occupation that takes a list of people 
as an argument and returns subset of the original list that only includes 
people who are firefighters.

3. Create a function called get_list_of_occupations that takes a list of 
people as an argument and returns a distinct list of occupations 
...
def get_list_of_zip_codes(list_of_people:list):
    # 1: set up an empty list before we start our 
    #    iteration process:
    # zip_codes = []    # create an empty list
    zip_codes = list()  # alternative syntax for creating an empty list
    for person in list_of_people:
        # 2. append each zip code to the list
        zip_codes.append(person[4]) 

    # 3. don't forget to return (OUTSIDE of the loop)
    return zip_codes

def filter_by_occupation(list_of_people:list):
    people_in_radio = []
    for person in list_of_people:
        if person[5] == 'Producer, radio':
            people_in_radio.append(person) 
    return people_in_radio

def filter_by_occupation_generic(list_of_people:list, role:str):
    subset_of_people = []
    for person in list_of_people:
        if person[5] == role:
            subset_of_people.append(person) 
    return subset_of_people

def get_list_of_occupations(list_of_people:list):
    occupations = []
    for person in list_of_people:
        if person[5] not in occupations:
            occupations.append(person[5])
    return occupations




zip_code_list = get_list_of_zip_codes(people)
print(get_list_of_zip_codes(people))

# print(get_list_of_zip_codes([
#     ['Tammy Spencer', '42022 Daniel Forest Apt. 201', 'Lake Amyhaven', 'Louisiana', '54479', 'Runner, broadcasting/film/video'],
#     ['Amy Meyer', '5558 Brown Street Apt. 718', 'Calebport', 'Wyoming', '96398', 'Administrator, charities/voluntary organisations'],
#     ['Michael Gutierrez', '560 Rhonda Isle Apt. 851', 'West Dakotaside', 'South Carolina', '92315', 'Location manager'],
# ]))
...
print()
print(filter_by_occupation(people))
print()
print(filter_by_occupation_generic(people, 'Naval architect'))
print()
print(filter_by_occupation_generic(people, 'IT trainer'))
print()
print(filter_by_occupation_generic(people, 'Acupuncturist'))
print()
print(get_list_of_occupations(people))